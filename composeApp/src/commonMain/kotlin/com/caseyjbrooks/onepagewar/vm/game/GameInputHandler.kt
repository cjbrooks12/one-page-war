...
internal class GameInputHandler(
    ...
    override suspend fun InputHandlerScope<
        ...
        is GameContract.Inputs.SetCurrentMove -> {
            updateState {
                val newCurrentMove = CurrentMove(
                    playerName = input.playerName,
                    arenaId = input.arenaId,
                    diceIndex = it.diceValues.lastIndex
                )

                if (newCurrentMove == it.currentMove) {
                    // we toggled the same spot on the same turn. Clear it out instead of selecting a new one
                    it.copy(
                        currentMove = null
                    )
                } else {
